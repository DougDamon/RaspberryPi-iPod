...
sWorkDirectory = './tmp/'
sMusicRootDirectory = './Music/'
sCodec = 'mp3'
sPlaylistExtension = 'm3u'
sPlaylistRootDirectory = "./Playlists/"
...
def getSongTrackNumber(YouTubeAlbum, YouTubeSong):
    # ytAlbum = YouTubeConnection.get_album(YouTubeSong['album']['id'])
    # print(YouTubeAlbum)
    if YouTubeAlbum is None:
        trackNumber = '00'
    else:
        i = 1
        for track in YouTubeAlbum['tracks']:
            if track['videoId'] == YouTubeSong['videoId']:
                if len(str(i)) == 1:
                    trackNumber = '0' + str(i)
                else:
                    trackNumber = str(i)
...
def getFileSystemDirectoryName(rootMusicDirectory, YouTubeSong):
    # Standard location is rootMusicDirectory/Artist/Album/
    # YouTube Video Location is rootMusicDirectory/YouTube/
    isYouTubeVideo = False
    if YouTubeSong['artists'][0]['name'] is None:
        artist = 'Unknown'
    else:
        artist = YouTubeSong['artists'][0]['name']

    if YouTubeSong['album'] is None:
        # assume this is a user loaded video
        isYouTubeVideo = True
        FileSystemDirectoryName = rootMusicDirectory + 'YouTube/'
    else:
        FileSystemDirectoryName = rootMusicDirectory + artist + '/'  + str(YouTubeSong['album']['name']) + '/'
...
def getFileSystemSongName(YouTubeAlbum, YouTubeSong):
    ...
    if YouTubeSong['album'] is None:
        # assume this is a user loaded video
        isYouTubeVideo = True
        sVidID = str(YouTubeSong['videoId'])

    else:
        album = YouTubeSong['album']['name']
        trackNumber = getSongTrackNumber(YouTubeAlbum, YouTubeSong)

    if isYouTubeVideo == False:
        fileSystemSongName = artist + '_' + album + '_' + trackNumber + '_' + songName
    else:
        fileSystemSongName = artist + '_' + songName + '_' + sVidID

    return fileSystemSongName
...
def newest_mp3_filename(directory):
...
def apply_id3_tags(YouTubeAlbum, YouTubeSong, downloadFile):
    try:
        currentSongTags = music_tag.load_file(downloadFile)
    except OSError as error :
        print(error)
        print('Error opening file: ' + downloadFile)
    currentSongTags['title'] = YouTubeSong['title']
    currentSongTags['artist'] = YouTubeSong['artists'][0]['name']
    if song['album'] is None:
        currentSongTags['album'] = 'Unknown' # YouTubeSong['title']
        currentSongTags['comment'] = 'Most likey user uploaded video.  No Song or Album information available'
        i = 0
        for imgLoc in song['thumbnails']:
        ...
        currentSongTags['album'] = YouTubeSong['album']['name']
        # album = YouTubeConnection.get_album(YouTubeSong['album']['id'])
        currentSongTags['totaltracks'] = YouTubeAlbum['trackCount']
        currentSongTags['year'] = YouTubeAlbum['year']
        try:
            currentSongTags['comment'] = YouTubeAlbum['description']
        except KeyError:
            currentSongTags['comment'] = 'Most likey user uploaded video.  No Song or Album information available'
        currentSongTags['albumartist'] = YouTubeAlbum['artists'][0]['name']
        # print(album.keys())
        # i = 1
        ...
        # i += 1
        currentSongTags['tracknumber'] = getSongTrackNumber(YouTubeAlbum, YouTubeSong)
        art = currentSongTags['artwork']
        # print(art)
        print(YouTubeAlbum['thumbnails'][-1])
        imgLoc = YouTubeAlbum['thumbnails'][-1]
        tmp = urlopen(imgLoc['url'])
        data = bytearray(tmp.read())
        currentSongTags['artwork'] = io.BytesIO(data).getvalue()
        # i = 0
    ...
    currentSongTags.save()
    return currentSongTags


# Press the green button in the gutter to run the script.
yt = YTMusic('oauth.json')

##ytdl = YouTube(str(input("Enter the URL of the video you want to download: \n>>")))
userPlayLists = yt.get_library_playlists()
# print(userPlayLists)
libraryPlaylists= yt.get_library_playlists()
...
# iPlaylistLength = iPodPlayList.len()
     if not os.path.exists(sPlaylistRootDirectory):
         os.makedirs(sPlaylistRootDirectory)
fPlayList = open(sPlaylistRootDirectory + currentPlaylistName + '.' +sPlaylistExtension,'w+')
fPlayList.write('#EXTM3U\r\n\r\n')
for i in iPodPlaylist:
    fPlayList.write(i + '\r\n\r\n')
